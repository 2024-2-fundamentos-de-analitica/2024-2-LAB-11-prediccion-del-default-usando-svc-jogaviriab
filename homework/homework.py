...
def load_data():
    """Carga los datasets de entrenamiento y prueba desde archivos comprimidos."""
...
def clean_data(df):
    """Limpia el dataset eliminando valores no válidos y transformando variables categóricas."""
    df_copy = df.copy()
    df_copy = df_copy.rename(columns={"default payment next month": "default"})
    df_copy = df_copy.drop(columns=["ID"])  # Elimina la columna de identificación
    df_copy = df_copy.loc[df["MARRIAGE"] != 0]  # Filtra valores no válidos en MARRIAGE
    df_copy = df_copy.loc[df["EDUCATION"] != 0]  # Filtra valores no válidos en EDUCATION
    df_copy["EDUCATION"] = df_copy["EDUCATION"].apply(lambda x: 4 if x >= 4 else x)  # Agrupa valores de EDUCATION
    df_copy = df_copy.dropna()  # Elimina valores nulos
    return df_copy
...
def create_pipeline(x_train):
    """Crea un pipeline de preprocesamiento y modelo SVM."""
...
def create_estimator(pipeline, x_train):
    """Configura y devuelve un GridSearchCV para optimizar los hiperparámetros del pipeline."""
    param_grid = {
...
def _create_output_directory(output_directory):
    """Crea el directorio de salida si no existe, eliminando archivos previos."""
...
def calculate_precision_metrics(dataset_type, y_true, y_pred):
    """Calcula métricas de precisión, recall, f1-score y balanced accuracy."""
    return {
        "type": "metrics",
